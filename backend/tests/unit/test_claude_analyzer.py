"""
Unit tests for the Claude analyzer pipeline.

Tests focus on the JSON parsing and robustness logic — the parts most likely
to break in production when the LLM returns unexpected output formats.
...
class TestIsRelevant:
    ...
    def test_returns_true_when_relevant(self):
        with patch("app.analysis.claude_analyzer._call_haiku", return_value='{"relevant": true}'):
            result = _is_relevant("Apple Q4 earnings beat estimates", "content", ["AAPL"])
        assert result is True

    def test_returns_false_when_not_relevant(self):
        with patch("app.analysis.claude_analyzer._call_haiku", return_value='{"relevant": false}'):
    ...
    def test_defaults_to_true_on_invalid_json(self):
        ...
        with patch("app.analysis.claude_analyzer._call_haiku", return_value="not json"):
            result = _is_relevant("title", "content", ["AAPL"])
        assert result is True

    def test_defaults_to_true_on_api_error(self):
        with patch("app.analysis.claude_analyzer._call_haiku", side_effect=Exception("API error")):
            result = _is_relevant("title", "content", ["AAPL"])
        assert result is True
...
class TestAnalyze:
    ...
    def test_parses_valid_json(self):
        with patch("app.analysis.claude_analyzer._call_sonnet", return_value=self.VALID_RESPONSE):
    ...
    def test_strips_markdown_code_fences(self):
        ...
        fenced = f"```json\n{self.VALID_RESPONSE}\n```"
        with patch("app.analysis.claude_analyzer._call_sonnet", return_value=fenced):
            result = _analyze("Apple guidance cut", "content", ["AAPL"], "US")
        assert result is not None
        assert result["sentiment_score"] == pytest.approx(-0.82)
    ...
    def test_returns_none_on_invalid_json(self):
        with patch("app.analysis.claude_analyzer._call_sonnet", return_value="Here is my analysis: blah blah"):
            result = _analyze("title", "content", ["AAPL"], "US")
        assert result is None

    def test_returns_none_on_empty_response(self):
        with patch("app.analysis.claude_analyzer._call_sonnet", return_value=""):
            result = _analyze("title", "content", ["AAPL"], "US")
        assert result is None

    def test_returns_none_on_api_error(self):
        with patch("app.analysis.claude_analyzer._call_sonnet", side_effect=Exception("rate limit")):
            result = _analyze("title", "content", ["AAPL"], "US")
        assert result is None

    def test_handles_positive_sentiment(self):
        positive = self.VALID_RESPONSE.replace("-0.82", "0.95")
        with patch("app.analysis.claude_analyzer._call_sonnet", return_value=positive):
            result = _analyze("title", "content", ["AAPL"], "US")
